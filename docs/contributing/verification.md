...
Read it with its boundary. The baseline check walks all 94 pins, including those
living in the seven outputs owned by stages that were **not** re-run; for those,
a PASS reads intact files rather than showing that their stages would reproduce
them. The reproduction claim rests on the 18 stages actually executed. A second
operator session corroborated the tree independently the same day — 94/94 pins
resolved and matched by walking each `json_path` directly, no runner and no
stage, so nothing was regenerated by the check — which confirms the state of the
tree, not reproduction. And the study tree's run manifest still describes the
2026-09-15 execution while 91 files under `outputs/` were rewritten, so manifest
and artefacts are no longer one set: take that tree's provenance from the two
flagship receipts together, not from the manifest alone.

The receipt reads `STALE` on today's `main`, and that is accounted for rather
than ignored. Nine watched files moved after it was recorded: the datagen agents
and three of the flagship's own scripts (#61), `CALIBRATION.md` (#59) and this
receipt's own `README.md` correction (#60). The last two are prose. #61 is a
refactor that pins its output-neutrality — a pinned parameter is a zero-width
distribution, so the draw is still consumed and the RNG stream does not shift —
and `tests/test_thermal_archetype.py` passes 13 cases on that tree, while
`results_baseline.json` is byte-identical across the move. That is evidence the
numbers should not have moved, not a re-run showing they did not; the remedy for
doubting it is a re-run, not a wider claim.
