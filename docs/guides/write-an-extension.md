...
Three files land under `/tmp/ext-demo/acme_backend/`: `acme_backend.py` (the
module — `descriptor` + `factory`), `pyproject.toml` (the entry-point wiring),
`test_acme_backend.py` (the conformance smoke test). The rest of this page
explains what each of those three files is for.
