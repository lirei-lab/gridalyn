...
| `openadr: https://openadr.org/ns#` | Returns 404; the OpenADR Alliance publishes no RDF vocabulary | **Removed 2026-09-11** — it was declared but never emitted. Since 2026-09-12 the `agent_interaction` capability emits `flexint:DemandResponseProgram` and `flexint:DemandResponseEvent`, gridalyn's own terms whose `aligns_with` property names the OpenADR 3.1.0 object each renders, carrying 3.1.0's own payload spellings |
