...
deployment's metering-point table (`--metering-points`), which has one row per point:
`usage_point_id`, the twin `load_id` it meters, and optionally `quantity`,
`resolution_minutes`, `source_dataset`. It emits one `cim:UsagePoint` per row
...
Two Parquet tables, one join. An edge is not a property of a node but a
...
unless it carries properties of its own (such as the `terminal` of a `CONNECTS` or
...
- `query_metered_buses(repository)` returns `usage_point_id → bus_id`. That is
  ...
  join is read off the model instead of written down a second time. The
