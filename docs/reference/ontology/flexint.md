...
Curtailment contracts and the flexibility-market entities (aggregators, portfolios, providers, offers, constraint zones); the agents, parties, roles and conversations of the interaction protocols, with demand-response programs and events; and the predicates of the network-impact surrogate graph.
...
| <a id="enablesContract"></a>`flexint:enablesContract` | `brick:Electric_Vehicle_Charging_Station` | `flexint:CurtailmentContract` | `ENABLES_CONTRACT` | Links an EV charging station to the curtailment contract it enables; distinct from efont:enables, which links a flexible operation to energy flexibility. |
...
Graphs built before the persistent namespaces carry these terms. The Turtle file marks each one `owl:deprecated`, and `resolve_deprecated_term` in `gridalyn/twin/semantic/profile.py` maps it to its replacement.
