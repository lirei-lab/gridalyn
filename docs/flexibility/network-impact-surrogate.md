...
`market verify-network-impact` validated the physics-backed predictions and
was retired with the chain above. The figures below are the last recorded S4
comparison it produced, kept as the reference result rather than as a
...
The validation report was generated by the retired
`market verify-network-impact`. Refresh the scenario-aware dashboard catalog:
