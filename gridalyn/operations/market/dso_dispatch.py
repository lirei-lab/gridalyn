"""Deprecated shim: DSO dispatcher moved to clearing.engine_mode.

``DSODispatcher`` now lives in
...
_EXPORTS = {"DSODispatcher"}


def __getattr__(name: str) -> object:
    """Resolve a moved dispatcher symbol from clearing.engine_mode."""
    if name in _EXPORTS:
        warnings.warn(
            f"import {name} from {_CANONICAL} "
            f"(gridalyn.operations.market.dso_dispatch.{name} is deprecated)",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    raise AttributeError(
        f"module 'gridalyn.operations.market.dso_dispatch' has no attribute {name!r}"
    )
