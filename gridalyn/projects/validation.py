...
def _check_path_contract(
    *,
    ...
    ``--check-artifacts``, so a path written repository-relative is reported as
    a doubled prefix naming its correction, instead of surfacing later as a
    misleading "missing required report".
