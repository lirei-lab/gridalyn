...
def run_single_realization(
    ...
    # Resolved once, outside the per-timestep closure: registry lookup is not
    # per-solve work, and the backend records `lightsim2grid=True` in its
    # descriptor rather than restating it at the call site.
    ...
    try:
        with np.errstate(
            ...
            run_timeseries(
                net, time_steps=range(time_steps), run=run_klu, verbose=False
            )
...
class PowerflowMonteCarloRunner:
    ...
    def __init__(
        self,
        input_file: str,
        cache_dir: str = "examples/generated/outputs",
    ...
    def _prepare_grid(self, force_rebuild: bool = False):
        ...
        if (
            os.path.exists(cache_pg)
            and os.path.exists(cache_pp)
        ...
        if (
            ...
            try:
                ...
                self.pg_graph = (
                    self.power_grid
                )  # Map abstract pg to local interface logic
        ...
        if (
            ...
            or not os.path.exists(cache_pg)
            or not os.path.exists(cache_pp)
        ):
            if (
                not force_rebuild
                and os.path.exists(cache_pg)
                ...
                print(
                    "====== Stage 1: Cached Grid Stale or Unversioned; Rebuilding ======"
                )
            else:
            ...
            building_data_output_filepath = os.path.join(
                self.cache_dir, "buildings_data.json"
            )
    ...
    def run_monte_carlo(
        ...
        print(
            f"Utilizing base grid Pickled topology {net_json_path} for parallel extraction."
        )
        ...
        generator_type = self.config.get("simulation", {}).get(
        ...
        with concurrent.futures.ProcessPoolExecutor(
            max_workers=concurrency, mp_context=ctx, max_tasks_per_child=1
        ) as executor:
            # We map the jobs
            futures = {
                executor.submit(run_single_realization, *args): args[0]
            ...
            for future in tqdm(
                ...
                desc="Parallel Stochastic Engines",
            ):
                try:
                    res = future.result()
                    results.append(res)
        ...
        self.pp_net.res_line = getattr(
            self.pp_net, "res_line", pd.DataFrame(index=self.pp_net.line.index)
        )
        self.pp_net.res_trafo = getattr(
            self.pp_net, "res_trafo", pd.DataFrame(index=self.pp_net.trafo.index)
        )
    ...
    def _verify_grid_stats(self):
        ...
        if len(line_loading) > 0:
            ...
            if overloaded_lines > 0:
                print(
                    f"  WARNING: {overloaded_lines} lines are overloaded (>100% capacity)!"
                )
            else:
