...
class NetworkObservation:
    ...
    def drop_missing(self) -> NetworkObservation:
        ...
        Returns:
            A new observation over the reported entries. ``converged`` and
            ``total_line_loss_mw`` are scalars and carry through unchanged.
        """
...
def observe_network(results: Any) -> NetworkObservation:
    ...
    Args:
        results: A solved network -- typically a ``pandapowerNet`` mutated in
            place by :func:`gridalyn.simulation.backends.registry.
            solve_power_flow`. A missing table or column is read as "not
            reported" rather than raising, because the summary writers this
            replaces already tolerated it.
