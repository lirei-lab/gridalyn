...
def create_node_payload(
    ...
    if node_type == "building":
        ...
        node = EnergyConsumer(
            id=node_id, x=x, y=y, type=node_type, p_mw=p_mw, q_mvar=q_mvar, **kwargs
        )
    elif node_type in ["lv_bus", "mv_bus", "hv_bus"]:
        node = ConnectivityNode(id=node_id, x=x, y=y, type=node_type, **kwargs)
    else:
        # Default fallback for intermediate nodes (like feeders)
        node = GridNodeBase(id=node_id, x=x, y=y, type=node_type, **kwargs)

    return node.model_dump(
        exclude={"id"}
    )  # We exclude ID because networkx uses it as the key
