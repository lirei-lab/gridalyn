...
def main() -> None:
    ...
    meta = script.read_json("outputs/json/imputer_comparison.json")
    ...
    for method, (label, color, style) in STYLE.items():
        ...
        ax.plot(
            sub["rho"], sub["loading_pct"], style, color=color, label=label, lw=lw, ms=5
        )
    ax.axhline(
    ...
    intr = meta["intrinsic_cv"]
    mc = meta["monte_carlo_rep"]
    rows = []
    ...
    pd.DataFrame(rows).to_csv(
        C.FIGURES_DIR / "imputer_comparison_summary.csv", index=False
    )
