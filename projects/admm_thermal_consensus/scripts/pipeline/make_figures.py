...
def main() -> None:
    ...
    ax.plot(
        hours, profiles[worst_key], "--", label=f"ADMM (rho={C.ILLUSTRATIVE_RHO})", lw=2
    )
    ...
    peak_at_limit = float(
        np.interp(
            C.TRANSFORMER_LOADING_LIMIT_PCT,
            fs["worst_transformer_loading_pct"],
            fs["peak_kw"],
        )
    )
    ...
    ax1.axhline(
        unc_peak, ls=":", color="gray", label=f"Uncoordinated peak ({unc_peak:.0f} kW)"
    )
    ax1.axhline(
        peak_at_limit,
        ls="--",
    ...
    ax1.axhline(
        ...
        ls=":",
        color="gray",
        label=f"{C.VOLTAGE_LOWER_PU} p.u. limit",
    )
    ax1.set_xlabel(r"Non-responsive fraction $\varrho$")
    ...
    ax2.plot(
        imp["rho"],
        imp["worst_transformer_loading_pct"],
        "s--",
        color="tab:red",
    ...
    # backing CSVs
    profiles.assign(hour=hours).to_csv(
        C.FIGURES_DIR / "aggregate_profiles.csv", index=False
    )
