...
def main():
    ...
    for col_name in df_base.columns:
        ...
        p_base_kw_realized, p_ev_kw_realized, p_tot_kw_realized = (
            context.realization_traces(col_name)
        )
        ...
        rebound_mwh = float(
            df_candidate.get(
                "rebound_kw", pd.Series(0.0, index=df_candidate.index)
            ).sum()
            * dt_h
            / 1000.0
        )
        ...
        balance_score = (
            0.0
            if max(soft_mwh, hard_mwh) <= 0.0
            else min(soft_mwh, hard_mwh) / max(soft_mwh, hard_mwh)
        )
        ...
        if (
            selected is None
            or row["interest_score"] > selected["metrics"]["interest_score"]
        ):
            selected = {
                "metrics": row,
                "df_res": df_candidate,
    ...
    selection_report = {
        ...
        "top_candidates": sorted(
            candidates, key=lambda row: row["interest_score"], reverse=True
        )[:10],
    }
    ...
    p_rebound = (
        df_res["rebound_kw"].values / 1000.0
        if "rebound_kw" in df_res.columns
        else np.zeros_like(p_unmanaged_total)
    )
    ...
    ax.fill_between(
        ...
        alpha=0.8,
        label="Managed Building Load",
    )
    ax.fill_between(
        t_hours,
        y1,
        y2,
        color=PALETTE["evs_managed"],
        alpha=0.8,
    ...
    ax.fill_between(
        ...
        y2,
        y3,
        color=PALETTE["soft_cls"],
        alpha=0.8,
        label="Soft-CLS (Firm DA Contract)",
    )
    ...
    if np.max(p_hard_cls) > 0:
        ax.fill_between(
            ...
            y3,
            y4,
            color=PALETTE["hard_cls"],
            alpha=0.8,
            label="Hard-CLS (Real-Time Recourse)",
        )
    ...
    ax.plot(
        ...
        ls=(0, (5, 2, 1, 2)),
        alpha=0.7,
        label=r"Realized Unmanaged Load ($\omega$)",
    )
