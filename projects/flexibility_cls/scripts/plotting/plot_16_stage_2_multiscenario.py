...
def main():
    ...
    for idx, col_name in enumerate(df_base.columns):
        ...
        ax_un.plot(
            t_hours, p_unmanaged_total, color="#888888", lw=0.8, ls="-", alpha=0.25
        )
    ...
    ax_un.plot(
        [], [], color="#888888", lw=1.5, alpha=0.6, label="Realized Unmanaged Scenarios"
    )
    ax_mg.plot(
        [],
        [],
        ...
        lw=1.5,
        alpha=0.8,
        label="Recourse Managed Scenarios (Soft+Hard CLS)",
    )
    ...
    ax_un.set_title(
        r"Multi-Scenario Recourse Validation: Dynamic Dispatch Performance",
        fontsize=16,
        weight="bold",
    )
