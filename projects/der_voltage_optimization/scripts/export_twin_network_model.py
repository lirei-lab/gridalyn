"""Export the DER feeder's base topology as a canonical gridalyn.twin NetworkModel.

Terminal, additive stage: rebuilds the same deterministic feeder
``build_der_feeder`` (``network_model.py``) declares -- topology only, no DER
dispatch scenario baked in, unlike ``build_der_feeder.py``'s exported CSVs,
which are a full-PV/zero-battery snapshot -- and exports it through
``PandapowerTopologyAdapter`` (no building-footprint layer; this feeder is
declared via ``RadialFeederSpec``, not building footprints, so buildings and
building_grid_connectivity are legitimately empty), so ``gridalyn.twin``'s
observation and semantic layers can load this project's network for the first
time via ``NetworkModelRepository.from_parquet(...)``.
...
from network_model import FEEDER_SPEC, build_der_feeder
...
def run_stage() -> dict[str, Any]:
    """Export the twin-native NetworkModel from the declared feeder spec.
    ...
    config_ref = script.write_json(
        ...
        dataclasses.asdict(FEEDER_SPEC),
    )
