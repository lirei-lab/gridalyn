"""Export the prosumer feeder's base topology as a canonical gridalyn.twin NetworkModel.

Terminal, additive stage: rebuilds the same deterministic feeder
``build_synthetic_feeder`` (``network_model.py``) declares and exports it
through ``PandapowerTopologyAdapter`` (no building-footprint layer; this
feeder is declared via ``RadialFeederSpec``, not building footprints, so
buildings and building_grid_connectivity are legitimately empty), so
``gridalyn.twin``'s observation and semantic layers can load this project's
network for the first time via ``NetworkModelRepository.from_parquet(...)``.
...
from network_model import FEEDER_SPEC, build_synthetic_feeder
