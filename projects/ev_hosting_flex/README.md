...
|---|---|---|
| `prepare_topology_cache` | Synthetic HQ twin via the public facade (240 V / 75 kVA / 2 % vk pole transformers) + feeder selection (idx-0, 6 homes) | `outputs/cache/*` |
...
| `replay_dr_program` | The mechanism as messages: replays the day-ahead call, the real-time backstop and the fair rotation through the SDK's `dr_program` protocol at every level of the declared adoption grid (`congestionEvPerHomeGrid`), and emits the message log the semantic graph's `agent_interaction` capability reads. Additive — it reads what `apply_curtailment_contracts` reads and moves no pinned number. Finding: at **≤ 1 EV/home nothing congests, so there is nothing to notify**; curtailment first appears at **1.5 EV/home** (2 windows, neither announced a day ahead), and at **2 EV/home only 2 of 14 windows** are. 3 EV/home exceeds the 16-EV pool and is reported as not representable, not truncated | `dr_replay.json`, `dr_replay_message_log.parquet` |
...
histogram runs 1:5, 2:9, 3:44, 4:87, 5:93, 6:96, 7:76, 8:54, 9:33, 10:43, since
the 2026-09-15 topology re-base capped a unit at ten homes). For that one unit,
at the same 71.25 kW rating and the same load:
