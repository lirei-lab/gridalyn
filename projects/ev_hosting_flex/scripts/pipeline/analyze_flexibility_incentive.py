...
def _optimize_bin(
    *, base, pool, bin_entry, n_ev, rating_kw, hod0, charger_kw, rating_series=None
    ...
    optimal = min(feasible, key=lambda k: feasible[k]["subsidy"]) if feasible else None
    return {
    ...
    *,
    ...
    for n in range(int(n_max), 0, -1):
        p95 = _bin_p95_loading(
            ...
            day_indices=day_indices,
            n_ev=n,
            n_enrolled=n if policy == "shift" else 0,
            policy=policy,
            rating_kw=rating_kw,
            hod0=hod0,
            charger_kw=charger_kw,
...
def _crossover_from_ceiling(bins: list[dict[str, Any]], target: float) -> float | None:
    ...
    for cold, warm in zip(ordered[:-1], ordered[1:], strict=True):
...
def derive_incentive(script: ProjectScript) -> dict[str, Any]:
    ...
    min_capped_ceiling = min(
        (
            b["shift_ceiling_ev_per_home"]
            for b in results
            if b["shift_ceiling_pool_capped"]
        ),
        default=float("inf"),
    )
    ...
    out_path_ref = script.write_json("outputs/json/flexibility_incentive.json", payload)
...
def _figures(payload: dict[str, Any], figures_dir: Path) -> list[Path]:
    ...
    ax1.plot(
        temps,
        [b["shift_ceiling_ev_per_home"] for b in bins],
        "o-",
        color="C2",
        label="optimal shift",
    )
    ax1.plot(
        temps,
        [b["uncontrolled_ceiling_ev_per_home"] for b in bins],
        "s--",
        color="C0",
        label="uncontrolled",
    )
    ax1.axhline(
        payload["target_ev_per_home"],
        color="k",
        ls=":",
        lw=1,
    ...
    for b in bins:
        ax2.scatter(
            b["mean_temp_c"],
            b["optimal_subsidy"] or 0.0,
            color=colors.get(b["optimal_policy"]),
            s=40,
        )
    if payload["crossover_temp_c"] is not None:
        ax2.axvline(
            payload["crossover_temp_c"],
            color="k",
            ls=":",
            lw=1.5,
...
def run_stage() -> dict[str, Any]:
    ...
    derived = derive_incentive(script)
    warnings = [
