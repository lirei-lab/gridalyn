...
def _adoption_voltage_stats(
    ...
    for pool in pools:
        for d in cold_days:
            ...
            ev_day = pool[:n_evs, sl].sum(axis=0) if n_evs > 0 else np.zeros(24, DTYPE)
...
def derive_voltage(script: ProjectScript) -> dict[str, Any]:
    ...
    out_path_ref = script.write_json("outputs/json/voltage_risk.json", payload)
