...
def test_data_manager_api(tmp_path):
    ...
    legacy_messages = [str(warning.message) for warning in legacy_warnings]
    assert any("DigitalTwinManager is legacy" in message for message in legacy_messages)
    assert any("DashboardExporter is legacy" in message for message in legacy_messages)
    ...
    try:
        ...
        if exc.__class__.__name__ in {
            ...
            try:
                ...
                print(
                    f"SKIP: Embedded FalkorDB/redislite backend is unavailable: {exc}"
                )
                return
    ...
    res = manager.falkor.execute_cypher(
        "MATCH (s:Substation)-[r]->(t) RETURN s.id, labels(s)[0], type(r)"
    )
    ...
    if os.path.exists(dashboard_public_dir):
        ...
        print(
            f"SUCCESS: Synchronized {len(files)} Twin Files specifically into the internal Dashboard server volume!"
        )
        print(files)
