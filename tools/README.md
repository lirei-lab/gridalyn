...
Thirteen scripts at this level. Eleven Python, two Node
...
| `pip_install_retry.py` (179 lines) | CI-wired (every install step in `ci.yml` and `pages.yml`) + pytest-gated (`tests/test_pip_install_retry.py`) | Runs a `pip install`, retrying only transport failures. On 2026-09-16 a cut package fetch reddened six jobs across five pull requests and `main`, every one green on a rerun of the same commit, and the byte counts were identical across the jobs of a single run — one upstream response cut, shared by whoever was installing. pip's own retries do not cover it: a body cut mid-download surfaces from the install phase as `OSError`. A resolution error is deliberately NOT retried, so a real break still fails in seconds. |
...
| `flagship_verify.py` (369 lines) | pytest-gated (`tests/test_flagship_verify.py`) | A shape-covering subset of the flagship `ev_hosting_flex` reproduce-and-pin protocol — 18 of 25 stages in ~34 min (measured 2026-09-17), skipping the four a clean run measured above ten minutes; the full cold regen (3 h 24 min, measured 2026-09-15) is operator-only, receipted separately. The heavy set is measured, not assumed: its previous sole member took five minutes and its exclusion left the subset running 3 stages. |
...
| `stage_profile.py` (547 lines) | pytest-gated (`tests/test_run_attestation.py`) | Reads a study's run manifest — which already records `started_at`/`ended_at` per stage — and reports where its wall time actually goes: per-stage share, the wave each stage sits in, and the speedup a concurrent runner could reach on the *measured* costs rather than on the DAG's shape. Flags a manifest that is partial (`--stage` filter), stale (workflow gained stages since the run), followed by partial runs, or whose fingerprinted artifacts have since been rewritten, instead of folding any of it into the totals. Recorded output: `docs/development/stage-profiles.md`. |
