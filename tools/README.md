...
| `flagship_verify.py` (366 lines) | pytest-gated (`tests/test_flagship_verify.py`) | A shape-covering subset of the flagship `ev_hosting_flex` reproduce-and-pin protocol — 16 of 23 stages in ~24 min, skipping the four a clean run measured above ten minutes; the full ~4 h cold regen is operator-only, receipted separately. The heavy set is measured, not assumed: its previous sole member took five minutes and its exclusion left the subset running 3 stages. |
