...
def test_der_voltage_optimization_runs_and_verifies_pandapower() -> None:
    ...
    optimization_report = (
        PROJECT_ROOT / "outputs" / "reports" / "der_voltage_optimization_report.json"
    )
    der_assets_path = PROJECT_ROOT / "outputs" / "data" / "der_assets.csv"
    sensitivity_path = (
        PROJECT_ROOT / "outputs" / "data" / "voltage_sensitivity_matrix.csv"
    )
    dispatch_path = PROJECT_ROOT / "outputs" / "operations" / "der_dispatch.csv"
    verification_path = (
        PROJECT_ROOT / "outputs" / "data" / "pandapower_verification.csv"
    )
    ...
    assert (
        optimization["summary"]["algorithm"]
        == "cvxpy_linearized_voltage_constrained_der_dispatch"
    )
    ...
    assert (
        optimization["summary"]["verified_max_voltage_before_pu"]
        > optimization["summary"]["verified_max_voltage_after_pu"]
    )
