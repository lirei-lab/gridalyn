...
_PIPELINE_DIR = _PROJECT_ROOT / "scripts" / "pipeline"
_GENERATE_STAGE = _PIPELINE_DIR / "generate_design_day_mc.py"

# The full downstream pipeline (stages 3->6) in strict dependency order. A later
# stage consumes the outputs of an earlier one, so they MUST be run sequentially
# (each asserted to exit 0 before the next). Re-running only stage 3 leaves the
# downstream JSON/parquet artifacts untouched, which would make the reserve/
# headline seal comparisons vacuous (CR-01/CR-02) — they would compare each
# downstream file against itself. Regenerating this whole chain makes those
# comparisons genuine run-vs-run reproducibility tripwires.
_DOWNSTREAM_STAGES = (
    _PIPELINE_DIR / "generate_design_day_mc.py",  # stage 3 — q_real/q_design/ev_pool
    _PIPELINE_DIR / "compute_congestion.py",  # stage 4 — firm_hosting.json
    _PIPELINE_DIR / "apply_flexibility_contracts.py",  # stage 5 — reserve/flexible
    _PIPELINE_DIR / "solve_twostage_program.py",  # stage 5b — feeds economics
    _PIPELINE_DIR / "compute_nonwires_economics.py",  # stage 6 — nonwires_economics
)
...
def _run_downstream_pipeline() -> None:
    """Re-run the full stage-3->6 pipeline in dependency order (fresh subprocesses).

    Mirrors ``_run_generation_stage`` exactly (an isolated interpreter per stage so
    the BLAS thread caps and ``SEED=42`` plumbing are exercised as the workflow runs
    them), but walks the whole ``_DOWNSTREAM_STAGES`` chain — generation (stage 3),
    congestion/firm hosting (stage 4), flexibility contracts (stage 5), the two-stage
    program (stage 5b), and non-wires economics (stage 6). Each stage consumes the
    prior stage's on-disk artifacts, so they are run strictly in order and each is
    asserted to exit 0 before the next is launched. This is what makes the reserve/
    headline seal comparisons non-vacuous: the downstream parquet/JSON artifacts are
    actually regenerated by a second ``SEED=42`` run before re-reading (CR-01/CR-02).

    Raises:
        AssertionError: If any stage exits non-zero (the chain is aborted there).
    """
    for stage in _DOWNSTREAM_STAGES:
        ...
        assert result.returncode == 0, (
            f"{stage.name} failed: " f"{result.stderr or result.stdout}"
        )
...
def test_reserve_traces_value_equal_after_parquet_readback() -> None:
    ...
    Reads the reserve/activation traces back from parquet, re-runs the FULL
    stage-3->6 downstream pipeline (not just generation — ``reserve_activation.parquet``
    is a stage-5 output, so re-running stage 3 alone would never rewrite it and the
    comparison would be vacuous, CR-02), and asserts the rounded float64 views are
    ``np.array_equal``. Parquet *file* bytes carry library/version metadata and are
    NOT asserted (Pitfall 4) — only the numeric values.
    """
...
def test_headline_scalars_stable_across_two_runs() -> None:
    ...
    Compares the citable scalar values from the data JSONs (never the timestamped
    ``*_report.json``) before and after a fresh run of the FULL stage-3->6 downstream
    pipeline. The three headline JSONs are outputs of stages 4/5/6, not stage 3, so
    re-running generation alone would leave them untouched and make the comparison
    vacuous (CR-01) — the full pipeline is re-run so the citable numbers are actually
    regenerated by a second ``SEED=42`` pass before re-reading.
    """
