...
class DashboardCatalogTest(unittest.TestCase):
    ...
    def test_write_dashboard_catalog_creates_parent_directory(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(json.loads(path.read_text())["report_id"], "digital_twin_dashboard_catalog")
    ...
    def test_build_dashboard_catalog_uses_network_repository_counts_as_fallback(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            pd.DataFrame(
                [{"transformer_id": "transformer:0", "hv_bus_id": "bus:1", "lv_bus_id": "bus:0"}]
            ).to_parquet(base / "grid_transformers.parquet")
            pd.DataFrame([{"building_id": "building:0", "load_id": "load:0"}]).to_parquet(
                base / "buildings.parquet"
            )
            pd.DataFrame(
                [{"building_id": "building:0", "load_id": "load:0", "load_bus_id": "bus:0"}]
        ...
        self.assertEqual(catalog["network_model"]["model_version_id"], "model:sha256:test")
        self.assertEqual(catalog["network_model"]["model_version"]["source_adapter"], "TestAdapter")
        ...
        self.assertEqual(catalog["scenarios"][0]["topology_counts"]["n_transformers"], 1)
