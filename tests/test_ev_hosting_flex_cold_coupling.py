...
def test_governed_cold_coupling_headline() -> None:
    ...
    firm cold-coupled 2 -> naive 3 (+50 %); the naive model underestimates the
    curtailment the flexibility contract must deliver ~2.8x, because it misses
    ...
    assert cold["firm_ev_count"] == 2
    assert naive["firm_ev_count"] == 3
    assert payload["firm_overestimate_percent"] == pytest.approx(50.0)
