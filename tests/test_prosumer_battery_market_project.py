...
def test_prosumer_battery_market_runs_end_to_end() -> None:
    ...
    feeder_report = (
        PROJECT_ROOT / "outputs" / "reports" / "synthetic_feeder_report.json"
    )
    market_report = (
        PROJECT_ROOT / "outputs" / "reports" / "prosumer_realtime_market_report.json"
    )
    prosumer_path = PROJECT_ROOT / "outputs" / "data" / "prosumers.csv"
    clearing_path = (
        PROJECT_ROOT / "outputs" / "operations" / "realtime_market_clearing.csv"
    )
    ...
    voltage_figure = (
        PROJECT_ROOT / "outputs" / "figures" / "synthetic_feeder_voltage_profile.png"
    )
    market_figure = (
        PROJECT_ROOT / "outputs" / "figures" / "prosumer_market_dispatch.png"
    )
    ...
    assert (
        market_payload["summary"]["algorithm"]
        == "rolling_horizon_uniform_price_auction"
    )
    ...
    assert (
        market_payload["summary"]["peak_import_before_mw"]
        > market_payload["summary"]["peak_import_after_mw"]
    )
    accepted = dispatch[dispatch["dispatch_mw"] > 0]
    assert not accepted.empty
    assert (
        accepted["settlement_price_usd_per_mwh"]
        == accepted["market_clearing_price_usd_per_mwh"]
    ).all()
