...
![The Trois-Rivières feeder held in the digital twin, its medium-voltage backbone coloured by electrical depth](docs/assets/feeder-hero.png)
...
![The bundled dashboard showing the digital twin: nodal voltage drop across the Trois-Rivières feeder, with grid health and ontology counts resolved from the model's own artifacts](docs/assets/dashboard-twin.jpg)

<sub>The browser dashboard under `dashboard/`, on the twin in
...
twin records, not a label added for the screenshot.</sub>
...
The flagship study carries 94 pinned metrics. Its current baseline puts firm
hosting at **11 EVs** (P05–P95: 10–12.6) and flexible hosting at **16** — a 45%
...
![Fleet triage across 540 pole transformers: the share that never binds, that a flexibility contract defers, that needs reinforcement, and that is constrained by the winter heating base, as EV adoption grows](docs/assets/study-fleet-triage.png)

<sub>The flagship's declared primary result, regenerated by its own workflow:
every pole transformer in the fleet classified at each adoption level. The
right panel is the honest part — as adoption clusters geographically, more
transformers never bind at all (7 to 58) and fewer need a flexibility contract
(107 to 51), while the count needing reinforcement rises. Clustering does not
reduce the problem, it concentrates it.</sub>
...
long-running — roughly six hours across 24 stages — and operator-verified via a
